...
<div class="showItem">
	...
	<? else: ?>
		<form method="post" id="shop-form" <? if ($this->shop['count'] == 0): ?> style="display: none;"<? endif ?> >
						...
						<? foreach($this->shop['products'] as $i => $pr): ?>
							...
							<tr class="cart_item row-<?=$pr['id'];?>">
								...
								<td class="cart_delete" data-title="Delete">
									<a href="#" title="Удалить" rel="nofollow" class="cart-delete" productid="<?=$pr['id'];?>">Удалить</a>
								</td>
							</tr>
						<? endforeach; ?>
					<? endif;?>
			...
			<div class="showItem">
				<input type="text" id="shop-name" name="data[order][user_name]" value="" placeholder="Ваше имя*" required />
				<br />
				<br />
				<input type="tel" id="shop-phone" name="data[order][user_phone]" value="" placeholder="Контактный телефон*" required />
				<br />
				<br />
				...
				<br />
				<br />
				<a href="#" rel="nofollow" onclick="jQuery('#shop-form').submit(); return false;" title="Оформить заказ">
