...
<div class="block_product">
	...
	<h1><a href="<?=$data->url?>"><?=$data->name?></a></h1>
	 <? if(!empty($data['brand']['name'])): ?><p>Марка автомобиля: <span><?=$data['brand']['name']?></span></p><? endif; ?>
	...
	<a data-productid="<?=$data->id?>" data-productname="<?=$data->name?>" data-productcount="1" data-proucturl="<?=$data->url?>" data-productimg="<?=$image?>" data-productprice="<?=$data->price?>" class="addtocard addtobasket"><img src="/images/basket.png" alt=""/><span class="basketButtonText">В корзину</span></a>
</div>