...
abstract class CBaseListView extends CWidget
{
    ...
    public function getMassSelect(){
		if(count($this->allActions)>0){

			echo '<div class="conteiner">
					<div class="filter block71">
					<div class="block4">
					<div class="select">
					<select id="massselect" name="action">';
                		for($i=0;$i<count($this->allActions);$i++){
                			echo '<option value="'.$this->allActions[$i]['value'].'">'.$this->allActions[$i]['name'].'</option>';
                		}
			echo '
					</select>
					</div>
					<div class="filter_btn">';
			echo CHtml::openTag('button', array('class'=>'button', 'id'=>'massbutton'));
			echo '<span class="btn_title">Выполнить</span>';
	...
	{
		$emptyText=$this->emptyText===null ? Yii::t('zii','В данном модуле ничего не найдено') : $this->emptyText;
