...
Yii::import('zii.widgets.CBaseListViewAdmin');
...
class CGridView extends CBaseListView
{
	...
	public function init()
	{
		...
		if(!isset($this->htmlOptions['class']))
			$this->htmlOptions['class']='grid-view ' . $this->gridsize;
        
       	if($this->jqurl)
			$this->htmlOptions['jqurl']=$this->jqurl;
    ...
    static function getDeleteButton($idfield='id', $visible=null){
		$array=array(
				'label'=>'', // text label of the button
				'url'=>"CHtml::normalizeUrl(array('delete', 'id'=>\$data->".$idfield."))",
				'imageUrl'=>false,  // image URL of the button. If not set or false, a text link is used
				'deleteConfirmation'=>"Вы уверены что хотите удалить страницу ?",
				'options' => array('class'=>'action_btn delete delete-one', 'title'=>'Удалить',), // HTML options for the button
		);
		if($visible){
	...
	static function getEditButton($idfield='id', $visible=null){
		$array=array(
				'label'=>'', // text label of the button
				'url'=>"CHtml::normalizeUrl(array('update', 'id'=>\$data->".$idfield."))",
				'imageUrl'=>false,  // image URL of the button. If not set or false, a text link is used
				'options' => array('class'=>'action_btn edit', 'title'=>'Редактировать'), // HTML options for the button
		);
		if($visible){
			$array['visible']=$visible;
		}
	...
	static function getViewButton($idfield='id', $visible=null){
		$array=array(
				'label'=>'', // text label of the button
				'url'=>"CHtml::normalizeUrl(array('view', 'id'=>\$data->".$idfield."))",
				'imageUrl'=>false,  // image URL of the button. If not set or false, a text link is used
				'options' => array('class'=>'action_btn view', 'title'=>'Просмотреть'), // HTML options for the button
		);
		if($visible){
			$array['visible']=$visible;
		}
	...
	static function getPublicButton($idfield='id', $visible=null){
		$array=array(
				'label'=>'', // text label of the button
				'url'=>"CHtml::normalizeUrl(array('public', 'id'=>\$data->".$idfield."))",
				'imageUrl'=>false,  // image URL of the button. If not set or false, a text link is used
				'options' => array('class'=>'action_btn public public-one', 'title'=>'Опубликовать/Снять с публикации'), // HTML options for the button
		);
		if($visible){
			$array['visible']=$visible;
		}
	...
	static function getBanButton($idfield='id', $visible=null){
		$array=array(
				'label'=>'', // text label of the button
				'url'=>"CHtml::normalizeUrl(array('main', 'id'=>\$data->".$idfield."))",
				'imageUrl'=>false,  // image URL of the button. If not set or false, a text link is used
				'options' => array('class'=>'action_btn public_front', 'title'=>'Установить главной'),
    ...
    static function getDeleteButtonImage($url = '', $idfield = 'id', $visible = null){
        if ($url == ''){
            $url = "CHtml::normalizeUrl('/admin/image/delete?id='.\$data->id)";
        }else{
            $url = "CHtml::normalizeUrl('$url?id='.\$data->id)";
        }
		$array=array(
				'label' => '', 
				'url' => $url,
    ...
    static function getPublicButtonImage($url = '', $idfield='id', $visible=null){
        if ($url == ''){
            $url = "CHtml::normalizeUrl('/admin/image/public?id='.\$data->id)";
        }else{
            $url = "CHtml::normalizeUrl('$url?id='.\$data->id)";
        }
    ...
    static function getCustomButton($url = '', $class){
        if ($url != ''){
            switch ($class){
                case 'public':
                    $opt_class = ' public public-one';
                    $opt_title = 'Опубликовать/Снять с публикации';
                    break;
                case 'delete':
                    $opt_class = ' delete delete-one';
                    $opt_title = 'Удалить';
                    break;
                default:
                    $opt_class = '';
    		...
    		$array=array(
    				...
    				'url'=> $url,
    				'imageUrl' => false,  
    				'options' => array('class'=>'action_btn' . $opt_class, 'title' => $opt_title), // HTML options for the button
    		);
    		return $array;
        }
	}
