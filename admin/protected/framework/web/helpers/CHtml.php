...
EOD;
    ...
    public static function activeImageField($model,$attribute,$htmlOptions=array())
	{
		...
		$hiddenOptions=isset($htmlOptions['id']) ? array('id'=>self::ID_PREFIX.$htmlOptions['id']) : array('id'=>false);
		return self::hiddenField($htmlOptions['name'],'',$hiddenOptions)
			. self::activeInputField('file',$model,$attribute,$htmlOptions). self::imageCustom($model,$attribute,$htmlOptions);
	}
    
    public static function imageCustom($model, $attribute, $htmlOptions=array())
	{   
        $html = '';
        
        if(!empty($model->$attribute)){
            $tumbs = $model->imageUpload->tumbs;
            reset($tumbs);
            $first_key = key($tumbs);
            $src = '';
            ...
            if(is_array($attr)){
                if(!empty($attr['path'])){
                    ...
                    if($first_key != ''){
                        if(is_file(HOME . $attr[$first_key]))    
                            $src = $attr[$first_key];
                        elseif(is_file(HOME . $attr['path']))
                            $src = $attr['path'];
                    }else{
                        if(is_file(HOME . $attr['path']))
                            $src = $attr['path'];
                    }
                    ...
                    if($src != ''){    
                        $html = 
							'<a class="fancybox" href="'. $attr['path'] . '">' .
								'<img src="' . $src . '" width="30" style="vertical-align: middle;" id="' . $attribute. '-' . $model->id . '"/>' .
							'</a>';
                        $html .= '&nbsp;<a href="#" class="delete-img" model="'.$class.'" objectid="'.$model->id.'" attr="'.$attribute.'">[x]</a>';
                    }
    ...
    public static function activeMultiDropDownList($model,$attribute,$data,$dataselect, $htmlOptions=array())
	{   
        ...
        $class = get_class($model);
        $html = '<select size="' . $size . '" multiple="multiple" name="' . $class . '[' . $attribute . '][]' . '" id="' . $class . '_' . $attribute .'">';
        foreach ($data as $k => $v){
            if(in_array($k, $dataselect)){
